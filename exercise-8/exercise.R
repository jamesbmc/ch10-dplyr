...
str(pulitzer)
# I am surprised the circulation is characters, not numeric. My guess is that the commas made those values interpreted as text.
...
Pulitzer.Prize.Change <- mutate(pulitzer, diff.1990.2003.to.2004.2014 = 
...
most.2014 <- pulitzer %>% 
  filter(max(Pulitzer.Prize.Winners.and.Finalists..2004.2014) == 
                      Pulitzer.Prize.Winners.and.Finalists..2004.2014) %>% 
  select(Newspaper)
...
# My % decreases are characters right now, so I will change them to numeric
pulitzer$Change.in.Daily.Circulation..2004.2013 <- str_replace_all(pulitzer$Change.in.Daily.Circulation..2004.2013, "%", "") %>% 
  as.numeric()

biggest.decrease.circulation <- pulitzer %>% 
